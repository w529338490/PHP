function anmSourceRecord() { 
	...
	if(s==null || s==""){ 
		...
		if(source!=null && source!=""&&source!="error"){ admanageType="B";	 }else{	 
			if(s.indexOf("http://www.yougou.com")!=-1){ 
			...
			if (qqLoginSource != null && qqLoginSource != ""
				...
				+ (new Date(0)).toUTCString() + ";path=" + path;
			}
		} 			
		document.cookie = "source"+"=;expires="+(new Date(0)).toUTCString()+";path="+path; 
	} 
	var exp = new Date(); 
	exp.setTime(exp.getTime() + 1000 * 60 * 60 * 24); 
	document.cookie = "admanageType=" + escape (admanageType) + ";expires=" + exp.toUTCString()+";path="+path; 
}
