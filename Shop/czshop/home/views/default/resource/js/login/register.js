...
function nextspan(obj){
	while(true){
		if(obj.nextSibling.tagName=="label"){
...
//设置三种状态
function setnregtips(obj,mess,check) {
	obj.onfocus=function(){
		nextspan(obj).className="Gray nreg_tips";
	...
	obj.onblur=function(){	
		...
		if(info[0]){
			nextspan(obj).className="nregtips";
			nextspan(obj).innerHTML="输入正确";	
		}else{
			nextspan(obj).className="nregtips2";
...
function validate(click){
	...
	setnregtips(username, '请正解输入EMAIL格式', function(val, mess){
		var info=[false, mess];		
	...
	setnregtips(userpwd,'用户密码应该为6-20位之间', function(val, mess){
		var info=[false, mess];
	...
	setnregtips(repwd,'输入的确认密码要和上面的密码一致,规则也要相同',function(val, mess){
		var info=[false, mess];
	...
	setnregtips(code, '输入内容必须和图片一致，看不清可以单击图片换一张', function(val, mess){
		var info=[false, mess];
		if(notnull(val)){
